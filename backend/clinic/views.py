...
from clinic.models import AdminUser, Appointment, DisplayMode, DisplayState, Doctor, DoctorQueueCounter, Service
from core.auth import admin_required, get_admin_from_request, is_super_admin
...
def _create_service(
    *,
    ...
    try:
        ...
        # Legacy Prisma schema may require `icon` without a DB default.
        message = str(exc).lower()
        if "icon" not in message and "default value" not in message:
            raise
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO `Service`
                    (`id`, `nameFr`, `nameAr`, `descriptionFr`, `descriptionAr`, `icon`, `active`, `createdAt`, `updatedAt`)
...
def admin_services(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        icon = optional_string(payload.get("icon"), "icon") or "activity"
        admin_email = optional_string(payload.get("adminEmail"), "adminEmail")
        admin_password = optional_string(payload.get("adminPassword"), "adminPassword")
    except ValidationError:
        return json_error("Invalid service data.", 400)

    if bool(admin_email) != bool(admin_password):
        return json_error("adminEmail and adminPassword must be provided together.", 400)
    if admin_email:
    ...
    try:
        with transaction.atomic():
            service = _create_service(
                service_id=payload.get("id") or cuid(),
                name_fr=name_fr,
                name_ar=name_ar,
                description_fr=description_fr,
                description_ar=description_ar,
            ...
            if admin_email:
                password_hash = bcrypt.hashpw((admin_password or "").encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
    ...
    return JsonResponse(
        {
            ...
            "serviceAdminCreated": bool(admin_email),
            "serviceAdminEmail": admin_email or None,
        }
    )
...
def admin_service_detail(_request: HttpRequest, service_id: str) -> JsonResponse:
    admin_user = get_admin_from_request(_request)
    if not is_super_admin(admin_user):
        return json_error("Forbidden", 403, "FORBIDDEN")
    ...
    try:
        with transaction.atomic():
            if AdminUser.objects.filter(service_id=service_id).exists():
                return json_error("Service has assigned service admins. Reassign or delete them first.", 409, "SERVICE_HAS_ADMINS")
...
def admin_doctors(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == "GET":
        try:
            ...
            if not is_super_admin(admin_user):
                doctors = doctors.filter(service_id=admin_user.service_id)
            return JsonResponse({"doctors": [doctor_to_dict(doctor) for doctor in doctors]})
        except DatabaseError:
            return json_error("Unable to list doctors.", 500, "LIST_FAILED")
    ...
    try:
        ...
        return json_error("Unable to create doctor.", 500, "CREATE_FAILED")
