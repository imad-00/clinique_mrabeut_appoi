...
from django.db.models import Count, Max as django_max, Q
...
from core.auth import admin_required, can_access_service, get_admin_from_request, is_super_admin
...
def _get_scoped_admin(request: HttpRequest) -> AdminUser:
    return get_admin_from_request(request)
...
def admin_appointments(request: HttpRequest) -> JsonResponse:
    admin_user = _get_scoped_admin(request)
    ...
    if request.method == "GET":
        ...
        if not is_super_admin(admin_user):
            filters["service_id"] = admin_user.service_id
        ...
        try:
            ...
            return json_error("Unable to list appointments.", 500, "LIST_FAILED")
    ...
    if not can_access_service(admin_user, service_id):
...
def admin_appointment_detail(request: HttpRequest, appointment_id: str) -> JsonResponse:
    ...
    if not appointment:
        return json_error("Appointment not found.", 404)
    if not can_access_service(admin_user, appointment.service_id):
        return json_error("Forbidden", 403, "FORBIDDEN")
...
def admin_call_next(request: HttpRequest) -> JsonResponse:
    ...
    doctor = Doctor.objects.filter(id=doctor_id).only("id", "service_id").first()
    if not doctor:
        return json_error("Doctor not found.", 404)
    if not can_access_service(admin_user, doctor.service_id):
        return json_error("Forbidden", 403, "FORBIDDEN")
    ...
    try:
        with transaction.atomic():
            ...
            waiting_candidates = list(
                ...
                .order_by("daily_queue_number", "doctor_queue_number", "arrived_at", "updated_at", "created_at")
...
def admin_call_specific(request: HttpRequest) -> JsonResponse:
    ...
    try:
        with transaction.atomic():
            ...
            if not target:
                return json_error("Appointment not found.", 404)
            if not can_access_service(admin_user, target.service_id):
                return json_error("Forbidden", 403, "FORBIDDEN")
...
@admin_required
@require_http_methods(["POST"])
def admin_reorder_waiting(request: HttpRequest) -> JsonResponse:
    admin_user = _get_scoped_admin(request)
    ...
    try:
        ...
        slot = parse_slot(payload.get("slot"), "slot")
        doctor_id = require_string(payload.get("doctorId"), "doctorId")
        ids_in_order = payload.get("appointmentIdsInOrder")
    except ValidationError:
        return json_error("Invalid request data.", 400)

    if not isinstance(ids_in_order, list) or not ids_in_order:
        return json_error("Invalid request data.", 400)
    if any(not isinstance(item, str) or not item.strip() for item in ids_in_order):
        return json_error("Invalid request data.", 400)
    ...
    try:
        with transaction.atomic():
            ...
            waiting_ids = {appt.id for appt in waiting}
            requested_ids = [item.strip() for item in ids_in_order]
            if set(requested_ids) != waiting_ids:
                return json_error("appointmentIdsInOrder must match waiting patients for this doctor/date/slot.", 400)

            existing_numbers: list[int] = []
            for appt in waiting:
                current = appt.daily_queue_number if appt.daily_queue_number is not None else appt.doctor_queue_number
                if current is not None:
                    existing_numbers.append(current)

            if len(existing_numbers) < len(waiting):
                max_used = (
                    Appointment.objects.select_for_update()
                    .filter(appointment_date__gte=day_start, appointment_date__lt=day_end)
                    .aggregate(
                        max_daily=django_max("daily_queue_number"),
                        max_doctor=django_max("doctor_queue_number"),
                    )
                )
                cursor = max(max_used.get("max_daily") or 0, max_used.get("max_doctor") or 0) + 1
                while len(existing_numbers) < len(waiting):
                    existing_numbers.append(cursor)
                    cursor += 1

            final_numbers = sorted(existing_numbers)
            by_id = {appt.id: appt for appt in waiting}

            base_temp = max(final_numbers) + 1_000_000
            for index, appointment_id in enumerate(requested_ids):
                appt = by_id[appointment_id]
                temp_number = base_temp + index
                appt.doctor_queue_number = temp_number
                appt.daily_queue_number = temp_number
                appt.save(update_fields=["doctor_queue_number", "daily_queue_number", "updated_at"])

            for index, appointment_id in enumerate(requested_ids):
                appt = by_id[appointment_id]
                number = final_numbers[index]
                appt.doctor_queue_number = number
                appt.daily_queue_number = number
                appt.save(update_fields=["doctor_queue_number", "daily_queue_number", "updated_at"])
    except Exception as exc:
        return json_error(f"Unable to reorder waiting patients: {exc}", 500)

    refreshed = (
        Appointment.objects.filter(id__in=requested_ids)
        .select_related("doctor", "service")
        .order_by("daily_queue_number", "doctor_queue_number", "updated_at")
    )
    return JsonResponse({"appointments": [appointment_to_dict(appt, include_relations=True) for appt in refreshed]})
