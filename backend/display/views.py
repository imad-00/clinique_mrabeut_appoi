...
def public_display_state(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return json_error("Unable to read display state.", 500, "DISPLAY_UNAVAILABLE")

    if since_date and display.updated_at and display.updated_at <= since_date:
    ...
    return JsonResponse(
        {
            ...
            "updatedAt": display.updated_at.isoformat() if display.updated_at else "",
        }
    )
...
def admin_display_state(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return json_error("Unable to update display state.", 500, "DISPLAY_UPDATE_FAILED")
