...
export default function AdminServicesPage() {
    ...
    const handleSave = async () => {
        if (!editing && (adminEmail || adminPassword) && (!adminEmail || !adminPassword)) {
            setFormError(
                locale === "ar"
                    ? "يرجى إدخال البريد وكلمة المرور معا أو تركهما فارغين"
                    : "Renseignez email + mot de passe ensemble, ou laissez les deux vides."
            )
            return
        }
                    ...
                    <DialogContent className="sm:max-w-md">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {!editing ? (
                                <>
                                    <div className="flex flex-col gap-2">
                                        <Label>{locale === "ar" ? "بريد مسؤول الخدمة (اختياري)" : "Email admin service (optionnel)"}</Label>
                                        <Input
                                            type="email"
                                            dir="ltr"
                                            value={adminEmail}
                                            onChange={(e) => setAdminEmail(e.target.value)}
                                            placeholder="service-admin@example.com"
                                        />
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <Label>{locale === "ar" ? "كلمة مرور مسؤول الخدمة (اختياري)" : "Mot de passe admin service (optionnel)"}</Label>
                                        <Input
                                            type="password"
                                            dir="ltr"
                                            value={adminPassword}
