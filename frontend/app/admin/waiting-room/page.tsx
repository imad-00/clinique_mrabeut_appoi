...
import {
  getAppointments,
  callNextForDoctor,
  callSpecificForDoctor,
  reorderWaitingPatients,
...
import { getAdminSession } from "@/src/lib/api/auth"
...
import { Monitor, Phone, Volume2, Search, CheckCircle, UserCheck, ArrowUp, ArrowDown } from "lucide-react"
...
export default function AdminWaitingRoomPage() {
  ...
  const [reorderDoctorId, setReorderDoctorId] = useState<string | null>(null)
  ...
  const getQueueSortValue = (appointment: Appointment) => {
    if (appointment.dailyQueueNumber !== null && appointment.dailyQueueNumber !== undefined) {
      return appointment.dailyQueueNumber
    }
    if (appointment.queueNumber !== null && appointment.queueNumber !== undefined) {
      return appointment.queueNumber
    }
    const createdAtTs = appointment.createdAt ? new Date(appointment.createdAt).getTime() : Number.MAX_SAFE_INTEGER
    return Number.isFinite(createdAtTs) ? createdAtTs : Number.MAX_SAFE_INTEGER
  }
  const sortByQueue = (a: Appointment, b: Appointment) =>
    getQueueSortValue(a) - getQueueSortValue(b)
  ...
  const openReorder = (doctorId: string) => {
    const waitingIds = appointments
      .filter((a) => a.doctorId === doctorId && a.status === "WAITING")
      .sort(sortByQueue)
      .map((a) => a.id)
    setReorderDoctorId(doctorId)
    setReorderIds(waitingIds)
  }
  ...
  const handleSaveReorder = async () => {
    if (!reorderDoctorId || reorderIds.length === 0) return
    setLoading(true)
    try {
      await reorderWaitingPatients({
        doctorId: reorderDoctorId,
        date: selectedDate,
        slot: selectedSlot,
        appointmentIdsInOrder: reorderIds,
      })
      setReorderDoctorId(null)
      setReorderIds([])
      await loadData()
    } finally {
      setLoading(false)
      ...
      <div className="flex flex-col gap-6">
              ...
              <CardContent className="p-5">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => openReorder(doctor.id)}
                    disabled={waiting.length < 2 || loading}
                    className="text-foreground bg-transparent"
                  >
                    {locale === "ar" ? "ترتيب" : "Ordre"}
                  </Button>
                </div>
              </CardContent>
            </Card>
                ...
                >
                  <div>
                    <span className="font-bold text-foreground">
                      #{apt.dailyQueueNumber ?? apt.queueNumber ?? "-"} - {apt.patientName}
                    </span>
                  </div>
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-foreground">
                {reorderDoctorId && doctorMap[reorderDoctorId]
                  ? (locale === "ar" ? "إعادة ترتيب المرضى - " : "Reordonner les patients - ") + getDocName(doctorMap[reorderDoctorId])
                  : ""}
              </DialogTitle>
              <DialogDescription className="sr-only">
                {locale === "ar" ? "تعديل ترتيب الانتظار" : "Modifier l'ordre de la file d'attente"}
              </DialogDescription>
            </DialogHeader>
            <div className="flex flex-col gap-2 max-h-80 overflow-y-auto">
              {reorderIds.map((appointmentId, index) => {
                const apt = appointments.find((item) => item.id === appointmentId)
                if (!apt) return null
                return (
                  <div
                    key={appointmentId}
                    className="flex items-center justify-between p-3 rounded-lg bg-clinic-soft"
                  >
                    <div>
                      <p className="font-semibold text-foreground">{apt.patientName}</p>
                      <p className="text-xs text-muted-foreground">
                        {locale === "ar" ? "الرقم" : "Numero"}: {index + 1}
                      </p>
                    </div>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setReorderDoctorId(null); setReorderIds([]) }}>
                {t("admin_cancel")}
              </Button>
              <Button
                onClick={handleSaveReorder}
                disabled={loading || reorderIds.length < 2}
