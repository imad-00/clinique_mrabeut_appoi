...
import type { AdminSessionUser } from "@/src/lib/auth-storage"
...
const navItems = [
  { key: "admin_dashboard" as const, href: "/admin/dashboard", icon: LayoutDashboard, roles: ["SUPER_ADMIN", "SERVICE_ADMIN"] },
  { key: "admin_services" as const, href: "/admin/services", icon: Stethoscope, roles: ["SUPER_ADMIN"] },
  { key: "admin_doctors" as const, href: "/admin/doctors", icon: UserCheck, roles: ["SUPER_ADMIN"] },
...
export function AdminLayout({ children }: { children: React.ReactNode }) {
      ...
      .then((session) => {
        ...
        const allowedPaths =
          user.role === "SUPER_ADMIN"
            ...
            : navItems.filter((item) => item.roles.includes("SERVICE_ADMIN")).map((item) => item.href)
  ...
  const visibleNavItems =
    sessionUser?.role === "SERVICE_ADMIN"
      ? navItems.filter((item) => item.roles.includes("SERVICE_ADMIN"))
      : navItems
