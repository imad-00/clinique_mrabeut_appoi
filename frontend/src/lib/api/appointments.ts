...
export async function reorderWaitingPatients(params: {
  doctorId: string
  date: string
  slot: AppointmentSlot
  appointmentIdsInOrder: string[]
}): Promise<Appointment[]> {
  const response = await apiClient.post<{ appointments: ApiAppointment[] }>(
    "/api/admin/waiting-room/reorder",
    {
      doctorId: params.doctorId,
      appointmentDate: params.date,
      slot: toApiSlot(params.slot),
      appointmentIdsInOrder: params.appointmentIdsInOrder,
    }
  )

  return response.appointments.map(mapAppointment)
}
